...
def eval_ppl(model, tokenizer, device=torch.device("cuda:0")):
    ...
    # Set dataset
    dataset = (
        "wikitext2"  # Dataset consisting of extracted sentences from Wikipedia articles
    )
...
def eval_ppl_wikitext(model, testenc, bs=1, device=None):
    ...
    for i in range(0, nsamples, bs):
        ...
        shift_logits = lm_logits[
            :, :-1, :
        ].contiguous()  # Example: [cat, sat, on, ???] -> [cat, sat, on]
        ...
        neg_log_likelihood = (
            loss.float() * model.seqlen * (j - i)
        )  # nll = loss * sequence_length * batch_size
    ...
    )  # ppl = exp(∑(nlls) / (num_samples * sequence_length))
