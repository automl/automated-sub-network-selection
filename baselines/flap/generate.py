...
model = torch.load(
    "/data/anyongqi/flap/llm_weights/c4_flap_p0.5_WIFV_AL-AM_llama_7b/pruned_model.pt",
    map_location=device,
)
model.eval()
...
for prompt in prompts:
    with torch.no_grad():
        ...
        if (
            input_ids[0][-1] == 2
        ):  # 2 is EOS, hack to remove. If the prompt is ending with EOS, often the generation will stop abruptly.
