...
class LoRALinear(LoRALayer):
    ...
    def set_sub_network(
        ...
        self.linear.set_sub_network(sub_network_in_features, sub_network_out_features, sampled_in_indices, sampled_out_indices)
    ...
    def get_lora_AB(self) -> torch.Tensor:
        ...
        if self.sampled_in_indices is not None and self.sampled_out_indices is not None:
            return (
                self.lora_B[self.sampled_out_indices, :]
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.sampled_in_indices is not None and self.sampled_out_indices is not None:
            lora = (
                self.lora_dropout(x)
                @ self.lora_A[:, self.sampled_in_indices].transpose(0, 1)
                @ self.lora_B[self.sampled_out_indices, :].transpose(0, 1)
            ) * self.scaling
        elif self.sampled_in_indices is not None:
            lora = (
                self.lora_dropout(x)
