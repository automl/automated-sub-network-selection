...
def test_block():
    ...
    cos_lit, sin_lit = build_rope_cache(litconfig.max_seq_len, n_elem=litconfig.rope_n_elem)
    ...
    out_large,_,_,_,_,_,_ = block(input, cos, sin, mask)
    ...
    out_small,_,_,_,_,_,_ = block(input[:, :, :32], cos, sin, mask)
