...
from lora_whittle.lora_mlps import LoRAGemmaMLP as GemmaMLP, LoRAGptNeoxMLP as GptNeoxMLP, LoRALLaMAMLP as LLaMAMLP
...
def test_GptNeoxMLP():
    ...
    gpt_neox_mlp.fc.linear.weight.data = torch.randn_like(gpt_neox_mlp.fc.linear.weight.data)
    gpt_neox_mlp.fc.linear.bias.data = torch.randn_like(gpt_neox_mlp.fc.linear.bias.data)
    gpt_neox_mlp.proj.linear.weight.data = torch.randn_like(gpt_neox_mlp.proj.linear.weight.data)
    gpt_neox_mlp.proj.linear.bias.data = torch.randn_like(gpt_neox_mlp.proj.linear.bias.data)
    gpt_neox_mlp.reset_super_network()
    out_large, _ = gpt_neox_mlp(input)
    assert out_large.shape == (8, 64)
    gpt_neox_mlp.set_sub_network(
        sub_network_n_embd=32, sub_network_intermediate_size=32 * 4
    )
    out_small, _ = gpt_neox_mlp(input[:8, :32])
    ...
    litgpt_neox_mlp_small.proj.bias.data = gpt_neox_mlp.proj.linear.bias.data[: config.n_embd]
...
def test_LLaMAMLP():
    ...
    # init weights and biases
    llama_mlp.fc_1.linear.weight.data = torch.randn_like(llama_mlp.fc_1.linear.weight.data)
    llama_mlp.fc_1.linear.bias.data = torch.randn_like(llama_mlp.fc_1.linear.bias.data)
    llama_mlp.fc_2.linear.weight.data = torch.randn_like(llama_mlp.fc_2.linear.weight.data)
    llama_mlp.fc_2.linear.bias.data = torch.randn_like(llama_mlp.fc_2.linear.bias.data)
    llama_mlp.proj.linear.weight.data = torch.randn_like(llama_mlp.proj.linear.weight.data)
    llama_mlp.proj.linear.bias.data = torch.randn_like(llama_mlp.proj.linear.bias.data)
    llama_mlp.reset_super_network()
    out_large, _ = llama_mlp(input)
    assert out_large.shape == (8, 64)
    llama_mlp.set_sub_network(
        sub_network_n_embd=32, sub_network_intermediate_size=32 * 4
    )
    out_small, _ = llama_mlp(input[:8, :32])
    assert out_small.shape == (8, 32)
...
def test_GemmaMLP():
    ...
    # init weights and biases
    gemma_mlp.fc_1.linear.weight.data = torch.randn_like(gemma_mlp.fc_1.linear.weight.data)
    gemma_mlp.fc_1.linear.bias.data = torch.randn_like(gemma_mlp.fc_1.linear.bias.data)
    gemma_mlp.fc_2.linear.weight.data = torch.randn_like(gemma_mlp.fc_2.linear.weight.data)
    gemma_mlp.fc_2.linear.bias.data = torch.randn_like(gemma_mlp.fc_2.linear.bias.data)
    gemma_mlp.proj.linear.weight.data = torch.randn_like(gemma_mlp.proj.linear.weight.data)
    gemma_mlp.proj.linear.bias.data = torch.randn_like(gemma_mlp.proj.linear.bias.data)
    gemma_mlp.reset_super_network()
    out_large, _ = gemma_mlp(input)
    assert out_large.shape == (8, 64)
    gemma_mlp.set_sub_network(
        sub_network_n_embd=32, sub_network_intermediate_size=32 * 4
    )
    out_small, _ = gemma_mlp(input[:8, :32])
    assert out_small.shape == (8, 32)
