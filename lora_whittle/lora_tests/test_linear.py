...
from lora_whittle.lora_linear import LoRALinear as Linear
...
def test_linear():
    ...
    linear.linear.weight.data = torch.randn_like(linear.linear.weight.data)
    linear.linear.bias.data = torch.randn_like(linear.linear.bias.data)
    linear.set_sub_network(sub_network_in_features=64, sub_network_out_features=16)
    ...
    small_layer.weight.data = linear.linear.weight.data[:16, :]
    small_layer.bias.data = linear.linear.bias.data[:16]
    out_small_layer = small_layer(input_features)

    large_layer = torch.nn.Linear(64, 32)
    large_layer.weight.data = linear.linear.weight.data
    large_layer.bias.data = linear.linear.bias.data
    out_large_layer = large_layer(input_features)

    small_large_layer = torch.nn.Linear(16, 32)
    small_large_layer.weight.data = linear.linear.weight.data[:32, :16]
    small_large_layer.bias.data = linear.linear.bias.data[:32]
