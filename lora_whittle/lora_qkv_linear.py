...
class LoRAQKVLinear(LoRALinear):
    ...
    def set_sub_network(
        ...
        sampled_in_indices=None,
        sampled_out_indices=None,
        ...
        self.linear.set_sub_network(
            sub_network_in_features, sub_network_out_features, sampled_in_indices, sampled_out_indices
        )
    ...
    def lora_ind(self) -> torch.Tensor:
        ...
        if not hasattr(self, "_lora_ind"):
            ...
            candidate_indices = range(self.sub_network_out_features) if self.sampled_out_indices is None else self.sampled_out_indices
            lora_ind = []
            if enable_q:
                q_ind = [
                    x
    ...
    def get_lora_AB(self) -> torch.Tensor:
        ...
        qkv_shapes = [s for s in qkv_shapes if s]
        if self.sampled_in_indices is not None and self.sampled_out_indices is not None:
            lora = self.conv1d(
                self.lora_A[:, self.sampled_in_indices].data.unsqueeze(
                    0
                ),  # (4, 128) -> (1, 4, 128)
                self.lora_B[
                    self.sampled_out_indices, :
                ].data.unsqueeze(-1),  # (256, 2) -> (256, 2, 1)
            ).squeeze(0)
        ...
        elif self.sampled_out_indices is not None:
            lora = self.conv1d(
                self.lora_A[:, : self.sub_network_in_features].data.unsqueeze(
                    0
                ),  # (4, 128) -> (1, 4, 128)
                self.lora_B[self.sampled_out_indices, :].data.unsqueeze(
                    -1
                ),  # (256, 2) -> (256, 2, 1)
            ).squeeze(0)
        else:
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.sampled_in_indices is not None:
            after_A = F.linear(
                self.lora_dropout(x),  # (64, 64, 128)
                self.lora_A[:, self.sampled_in_indices],
            )  # (4, 128) -> (64, 64, 4)
        else:
            after_A = F.linear(
        ...
        if self.sampled_out_indices is not None:
            after_B = self.conv1d(
                ...
                self.lora_B[self.sampled_out_indices, :].unsqueeze(-1),  # (256, 2) -> (256, 2, 1)
            ).transpose(
                -2, -1
