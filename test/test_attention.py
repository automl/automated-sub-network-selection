...
def test_attention(attention_config):
    ...
    out_large,_ = attention(input, mask=mask, cos=cos, sin=sin)
    ...
    if attention.qkv_indices is not None:  
        ...
        print(attention.attn.weight.data[attention.qkv_indices,:][:,0:config.n_embd].size())
        assert torch.all(lit_attention_small.attn.weight.data == attention.attn.weight.data[attention.qkv_indices,:][:,0:config.n_embd])
