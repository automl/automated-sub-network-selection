...
def test_gpt_sampled():
    ...
    gpt.set_sub_network(
        ...
        sampled_layer_indices= list(sorted(random.sample(range(gpt.config.n_layer), 1))),
    ...
    for i, block in enumerate(lit_gpt_small.transformer.h):
        ...
        block.attn.attn.weight.data = block_orig.attn.attn.weight.data[block_orig.attn.qkv_indices,:][:,gpt.sampled_embd_indices]
        block.attn.attn.bias.data = block_orig.attn.attn.bias.data[
