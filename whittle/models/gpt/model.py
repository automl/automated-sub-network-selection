...
class GPT(nn.Module):
    ...
    def __init__(self, config: Config) -> None:
        ...
        self.sampled_embed_dim_indices = None
    ...
    def set_sub_network(
        ...
        if self.config.n_query_groups == 1:
            ...
            self.sub_network_num_heads = sub_network_num_heads if sub_network_num_heads is not None else self.config.n_head
        if self.config.n_head != self.config.n_query_groups:
            self.sub_network_num_heads = sub_network_num_heads if sub_network_num_heads is not None else self.config.n_head
            self.sub_network_query_groups = sub_network_query_groups if sub_network_query_groups is not None else self.config.n_query_groups
        else:
            self.sub_network_query_groups = sub_network_query_groups if sub_network_query_groups is not None else self.config.n_head
        ...
        assert self.sub_network_n_embd == len(sampled_embd_indices) if sampled_embd_indices is not None else True
        assert self.sub_network_intermediate_size == len(sampled_intermediate_indices) if sampled_intermediate_indices is not None else True
        assert self.sub_network_num_heads == len(sampled_head_indices) if sampled_head_indices is not None else True
        assert self.sub_network_query_groups == len(sampled_query_group_indices) if sampled_query_group_indices is not None else True
        assert self.sub_network_head_size == len(sampled_head_size_indices) if sampled_head_size_indices is not None else True
        assert self.sub_network_n_layers == len(sampled_layer_indices) if sampled_layer_indices is not None else True
        self.transformer.wte.set_sub_network(self.sub_network_n_embd, sampled_embd_indices) 
        self.transformer.ln_f.set_sub_network(self.sub_network_n_embd, sampled_embd_indices)

        if sampled_layer_indices is not None:
            for j in sampled_layer_indices:
                self.transformer.h[j].set_sub_network(
        ...
        self.lm_head.set_sub_network(
            self.sub_network_n_embd, self.config.padded_vocab_size, sampled_embd_indices
        )
    ...
    def forward(
        ...
        self.intermediate_out["block_0"] = x.detach()
        if self.sampled_layer_indices is not None:
            for j in self.sampled_layer_indices:
                ...
                (
                    ...
                    norm2,
                    mlp_out_embd,
                    attention_output_embd,
                ) = block(x, cos, sin, mask, input_pos)
                self.intermediate_out[f"block_{j+1}"] = x.detach()
                self.intermediate_out[f"block_before_attn_{j+1}"] = norm1.detach()
                self.intermediate_out[f"block_attn_{j+1}"] = attention_output_embd.detach()
                self.intermediate_out[f"block_mlp_{j+1}"] = mlp_out_embd.detach()
                ...
                self.intermediate_out[f"embd_norm1_{j}"] = norm1.detach()
                self.intermediate_out[f"embd_norm2_{j}"] = norm2.detach()
        else:
            for i in range(self.sub_network_n_layers):
                block = self.transformer.h[i]
                ...
                #self.intermediate_out[f"block_before_attn_{i+1}"] = norm1.detach()
                #self.intermediate_out[f"block_attn_{i+1}"] = attention_output_embd.detach()
                #self.intermediate_out[f"block_mlp_{i+1}"] = mlp_out_embd.detach()
                ...
                self.intermediate_out[f"embd_norm1_{i}"] = norm1.detach()
                self.intermediate_out[f"embd_norm2_{i}"] = norm2.detach()
        x = self.transformer.ln_f(x)
        self.intermediate_out[f"embd_norm_f"] = x.detach()
